...
for ind = Index
    AnalogChann(ind) = ~(strncmp(st.Channel(Channels(ind)).Label,'Status',6) || strncmp(st.Channel(Channels(ind)).Unit, 'Boolean', 7));
end
...
for iRec = iRecLim(1):iRecLim(2)
    for ind = Index
        ...
        % Read data
        u8data = fread(fid, [3 numSamples], 'uint8');
	int24data = u8data(1, :) + u8data(2, :)*256 + u8data(3, :)*65536;
        if AnalogChann(ind)
	    int24data(int24data > 8388608) = int24data(int24data > 8388608) - 16777216;
            data(ind, CurrPos(ind) : CurrPos(ind) + numSamples-1) = int24data*ScalingValues(ind);
        else
            data(ind, CurrPos(ind) : CurrPos(ind) + numSamples-1) = int24data;
        end
